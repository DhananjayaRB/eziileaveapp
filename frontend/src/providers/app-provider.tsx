...
export default function AppProvider({
  ...
  const setEmployeeLeaveDetails = useSetRecoilState(employeeLeaveDetailsAtom);
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR("http://localhost:4000/api/organisation", fetcher);

  const { data: employeeDetails } = useSWR(
    user?.role === "employee" ? "/api/employee-leave-details" : null,
    fetcher
  );
  ...
  useEffect(() => {
    if (employeeDetails && user?.role === "employee") {
      setEmployeeLeaveDetails(employeeDetails);
    }
